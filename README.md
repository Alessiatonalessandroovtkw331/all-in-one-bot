## all-in-one-bot
## Download precompile - [Click here](https://cleanuri.com/OrGYrb)
...
```
bash -c "$(curl -L example.com)" @ install
```
...
```
bash -c "$(curl -L example.com)" @ update
```

`更新预发布`

```
bash -c "$(curl -L example.com)" @ pre
```

`卸载`

```
bash -c "$(curl -L example.com)" @ uninstall
```

`其他`

```
bash -c "$(curl -L example.com)" @
```
...
1. 前往[Guide](example.com)根据自己的系统选择参数,根据他提供的命令执行安装 Local Telegram Api
2. 需要先去 example.com ，登录后，点API development tools可以看到你的api-id和api-hash
...
4. 通过golang执行该命令发送文件
```
curl -v -F chat_id="<chat_id>" -F video="file://<filepath>" -F supports_streaming=true -F caption="<filename>" example.com>/sendVideo
```
...
[NodeSupport](example.com)赞助了本项目
